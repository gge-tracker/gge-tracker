...
async function createOuterRealmsInstance(): Promise<void> {
  ...
  try {
    const response = await generic.fetchUrl(url, body);
    console.log('Outer Realms socket server added successfully:', response.data);
    setTimeout(() => {
      console.log('Exiting in 60 seconds...');
      process.exit(1);
    }, 60 * 1000);
  } catch (error) {
    console.error('Error adding Outer Realms socket server:', error);
  }
}
