...
export abstract class ApiRiftRaid implements ApiHelper {
  ...
  private static readonly ZONE_PREFIXES: Record<RiftRaidGame, string> = {
    ep: 'EmpireEx',
    e4k: 'EmpirefourkingdomsExGG',
  };
  ...
  public static async getAllianceAnalysis(request: express.Request, response: express.Response): Promise<void> {
    try {
      ...
      const origin = allianceId ? ApiRiftRaid.rankingOriginOf(ApiHelper.getCountryCode(String(allianceId))) : null;
  ...
  private static gameOf(request: express.Request): RiftRaidGame {
    const server = ApiHelper.ggeTrackerManager.getServerDefinition(String(request['language']));
  ...
  private static zoneOf(game: RiftRaidGame, serverId: number): string {
    const prefix = ApiRiftRaid.ZONE_PREFIXES[game];
    return serverId === 1 ? prefix : `${prefix}_${serverId}`;
  }

  private static serversByZone(): Map<string, IServerDefinition> {
    return new Map(
      ApiHelper.ggeTrackerManager.getPublicServerDefinitions().map((definition) => [definition.zone, definition]),
    );
  }

  private static rankingOriginOf(
    code: string,
  ): { game: RiftRaidGame; serverId: number; server: IServerDefinition } | null {
    const server = ApiHelper.ggeTrackerManager.getPublicServerDefinitions().find((entry) => entry.code === code);
    const match = server ? /^(EmpireEx|EmpirefourkingdomsExGG)(?:_(\d+))?$/.exec(server.zone) : null;
    if (!server || !match) return null;
    return {
      game: match[1] === ApiRiftRaid.ZONE_PREFIXES.e4k ? 'e4k' : 'ep',
      serverId: match[2] ? Number(match[2]) : 1,
      server,
    };
  }

  private static toAlliances(game: RiftRaidGame, rows: RiftRaidAllianceRow[]): RiftRaidAlliance[] {
    const servers = ApiRiftRaid.serversByZone();
    return rows.map((row) => {
      const server = servers.get(ApiRiftRaid.zoneOf(game, row.server_id));
