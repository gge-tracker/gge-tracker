...
export const CATALOG: Endpoint[] = [
  ...
  { id: 'server-renames-filtered', snapshot: 'fields', method: 'GET', scope: 'protected', path: (s) => '/server/renames' + q({ page: 1, search: s.renamePlayerName ?? 'a', searchType: 'player', showType: 'players', allianceId: s.allianceId }), okStatuses: [200, 400], needs: ['server', 'alliance'] },
  ...
  { id: 'stats-player-event-duration', snapshot: 'fields', method: 'GET', scope: 'public', path: (s) => `/statistics/player/${s.playerId ?? '1'}/player_event_nomad_history/30`, okStatuses: [200, 400, 404], needs: ['server', 'player'] },
  ...
  },
  {
    ...
    method: 'GET',
    scope: 'protected',
    path: () => '/export/players' + q({ limit: 50 }),
    okStatuses: [200],
    shapeKeys: ['server', 'data_version', 'fields', 'page', 'players'],
    needs: ['server'],
    fuzzQuery: ['cursor', 'fields', 'limit', 'updated_since', 'alliance_id', 'active', 'format'],
    cases: [
      { label: 'default projection is the id triple', path: () => '/export/players' + q({ limit: 5 }), expect: [200] },
      { label: 'every column at once', path: () => '/export/players' + q({ limit: 5, fields: 'all' }), expect: [200] },
      { label: 'an unknown field is refused', path: () => '/export/players' + q({ limit: 5, fields: 'player_id,nope' }), expect: [400] },
      { label: 'a forged cursor is refused', path: () => '/export/players' + q({ cursor: 'not-a-cursor' }), expect: [400] },
      { label: 'limit above the cap is refused', path: () => '/export/players' + q({ limit: 5001 }), expect: [400] },
      { label: 'limit below one is refused', path: () => '/export/players' + q({ limit: 0 }), expect: [400] },
      { label: 'a non-integer limit is refused', path: () => '/export/players' + q({ limit: 1.5 }), expect: [400] },
      { label: 'an unparseable updated_since is refused', path: () => '/export/players' + q({ updated_since: 'yesterday' }), expect: [400] },
      { label: 'an unknown format is refused', path: () => '/export/players' + q({ format: 'csv' }), expect: [400] },
      { label: 'ndjson is served', path: () => '/export/players' + q({ limit: 5, format: 'ndjson' }), expect: [200] },
      { label: 'active players only', path: () => '/export/players' + q({ limit: 5, active: 1 }), expect: [200] },
      { label: 'restricted to one alliance', path: (s) => '/export/players' + q({ limit: 5, alliance_id: s.allianceId }), expect: [200] },
      { label: 'a stale ETag still returns the body', path: () => '/export/players' + q({ limit: 5 }), expect: [200], headers: () => ({ 'If-None-Match': 'W/"nope"' }) },
      { label: 'If-None-Match * is answered 304', path: () => '/export/players' + q({ limit: 5 }), expect: [304], headers: () => ({ 'If-None-Match': '*' }) },
    ],
    semantic: {
      collection: 'players',
      idField: 'player_id',
  ...
  },
  {
    ...
    method: 'GET',
    scope: 'protected',
    path: () => '/export/alliances' + q({ limit: 50 }),
    okStatuses: [200],
    shapeKeys: ['server', 'data_version', 'fields', 'page', 'alliances'],
    needs: ['server'],
    fuzzQuery: ['cursor', 'fields', 'limit', 'only_populated'],
    cases: [
      { label: 'aggregates are joined on request', path: () => '/export/alliances' + q({ limit: 5, fields: 'alliance_name,player_count,might_current' }), expect: [200] },
      { label: 'every column at once', path: () => '/export/alliances' + q({ limit: 5, fields: 'all' }), expect: [200] },
      { label: 'an unknown field is refused', path: () => '/export/alliances' + q({ limit: 5, fields: 'nope' }), expect: [400] },
      { label: 'empty alliances can be dropped', path: () => '/export/alliances' + q({ limit: 5, only_populated: 1 }), expect: [200] },
      { label: 'ndjson is served', path: () => '/export/alliances' + q({ limit: 5, format: 'ndjson' }), expect: [200] },
    ],
    semantic: {
      collection: 'alliances',
      idField: 'alliance_id',
  ...
  },
  {
    ...
    method: 'GET',
    scope: 'protected',
    path: () => '/export/castles' + q({ limit: 50 }),
    okStatuses: [200],
    shapeKeys: ['server', 'page', 'castles'],
    needs: ['server'],
    fuzzQuery: ['cursor', 'limit'],
    cases: [
      { label: 'ndjson is served', path: () => '/export/castles' + q({ limit: 5, format: 'ndjson' }), expect: [200] },
      { label: 'a forged cursor is refused', path: () => '/export/castles' + q({ cursor: '!!!' }), expect: [400] },
    ],
  },
  {
    id: 'changes',
    ...
    method: 'GET',
    scope: 'protected',
    path: () => '/changes' + q({ since: '2000-01-01T00:00:00.000Z', limit: 50 }),
    okStatuses: [200],
    shapeKeys: ['server', 'types', 'page', 'changes'],
    needs: ['server'],
    fuzzQuery: ['cursor', 'since', 'types', 'player_ids', 'alliance_ids', 'limit'],
    cases: [
      { label: 'defaults to the last 24 hours', path: () => '/changes', expect: [200] },
      { label: 'one type only', path: () => '/changes' + q({ since: '2000-01-01T00:00:00.000Z', types: 'membership' }), expect: [200] },
      { label: 'an unknown type is refused', path: () => '/changes' + q({ types: 'membership,nope' }), expect: [400] },
      { label: 'a forged cursor is refused', path: () => '/changes' + q({ cursor: 'nope' }), expect: [400] },
      { label: 'an unparseable since is refused', path: () => '/changes' + q({ since: 'last tuesday' }), expect: [400] },
      { label: 'limit above the cap is refused', path: () => '/changes' + q({ limit: 1001 }), expect: [400] },
      { label: 'restricted to one player', path: (s) => '/changes' + q({ since: '2000-01-01T00:00:00.000Z', player_ids: s.playerId }), expect: [200] },
      { label: 'restricted to one alliance', path: (s) => '/changes' + q({ since: '2000-01-01T00:00:00.000Z', alliance_ids: s.allianceId }), expect: [200] },
      { label: 'a malformed subject id is refused', path: () => '/changes' + q({ player_ids: 'abc' }), expect: [400] },
    ],
  },
  {
    id: 'player-profile',
    method: 'GET',
    scope: 'public',
    path: (s) => `/players/${s.playerId ?? '1'}/profile`,
    okStatuses: [200, 400, 404],
    shapeKeys: ['server', 'player'],
    needs: ['server', 'player'],
    fuzzQuery: ['include'],
    cases: [
      { label: 'narrowed to one section', path: (s) => `/players/${s.playerId ?? '1'}/profile` + q({ include: 'rank' }), expect: [200, 400, 404] },
      { label: 'the player row alone', path: (s) => `/players/${s.playerId ?? '1'}/profile` + q({ include: 'none' }), expect: [200, 400, 404] },
      { label: 'an unknown section is refused', path: (s) => `/players/${s.playerId ?? '1'}/profile` + q({ include: 'nope' }), expect: [400] },
      { label: 'an unparseable id is refused', path: () => '/players/not-an-id/profile', expect: [400] },
      { label: 'If-None-Match * is answered 304', path: (s) => `/players/${s.playerId ?? '1'}/profile`, expect: [304], headers: () => ({ 'If-None-Match': '*' }) },
    ],
  },
  {
    id: 'alliance-profile',
    method: 'GET',
    scope: 'public',
    path: (s) => `/alliances/${s.allianceId ?? '1'}/profile`,
    okStatuses: [200, 400, 404],
    shapeKeys: ['server', 'alliance', 'statistics'],
    needs: ['server', 'alliance'],
    fuzzQuery: ['include'],
    cases: [
      { label: 'members only', path: (s) => `/alliances/${s.allianceId ?? '1'}/profile` + q({ include: 'members' }), expect: [200, 400, 404] },
      { label: 'an unknown section is refused', path: (s) => `/alliances/${s.allianceId ?? '1'}/profile` + q({ include: 'nope' }), expect: [400] },
      { label: 'an unknown alliance is a 404', path: () => '/alliances/99999999999/profile', expect: [400, 404] },
  ...
  },
  {
    ...
    body: (s) => [s.playerId ?? '1'],
    okStatuses: [200, 400],
    shapeKeys: ['requested', 'resolved', 'servers', 'missing', 'players'],
    needs: ['server', 'player'],
  },
  {
    id: 'bulk-alliances',
    ...
    scope: 'public',
    path: () => '/bulk/alliances',
    body: (s) => ({ ids: [s.allianceId ?? '1'] }),
    okStatuses: [200, 400],
    shapeKeys: ['requested', 'resolved', 'servers', 'missing', 'alliances'],
    needs: ['server', 'alliance'],
  },
];
