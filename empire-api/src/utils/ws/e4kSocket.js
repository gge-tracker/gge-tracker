...
class E4kSocket {
  ...
  async log(message, ...args) {
    ...
    console.log(`[${day} ${time}] [E4K][${this.serverHeader}] ${message}`, ...args);
  }
  ...
  async sendLoginMessage() {
    this.socket.sendJsonCommand("core_lga", {NM: this.username, PW: this.password, L: "fr", AID: "1760000000000000000", DID: "5", PLFID: "3", ADID: "null", AFUID: "ggetracker", IDFV: "null"});
  }

  async sendRegisterMessage() {
    this.socket.sendJsonCommand("core_reg", {PN: this.username, PW: this.password, MAIL: `${this.username}-${Math.floor(Math.random() * 99999)}@mail.com`, LANG: "fr", AID: "1760000000000000000", DID: "5", PLFID: "3", ADID: "null", AFUID: "appsFlyerUID", IDFV: "null", REF: ""});
  }
    ...
    try {
      ...
      if (lgaResponse.payload.status === E4kEnumLoginStatus.SUCCESS) {
        ...
        if (regResponse.payload.status === E4kEnumLoginStatus.SUCCESS) {
          ...
          this.sendLoginMessage();
          const lgaLoginResponse = await this.socket.waitForJsonResponse("core_lga");
          if (lgaLoginResponse.payload.status === E4kEnumLoginStatus.SUCCESS) {
            this.pingAndCheck();
          } else {
            this.log("❌ [connect] Login after registration failed with status:", lgaLoginResponse.payload.status, "retrying in 5 minutes...");
            setTimeout(() => {
              this.restart();
            }, 5 * 60 * 1000);
          }
        } else {
          this.log("❌ [connect] Registration failed with status:", regResponse.payload.status, "retrying in 5 minutes...");
          setTimeout(() => {
            this.restart();
          }, 5 * 60 * 1000);
        }
      } else {
        this.log("❌ [connect] Login failed with status:", lgaResponse.payload.status, "retrying in 5 minutes...");
        setTimeout(() => {
          this.restart();
        }, 5 * 60 * 1000);
      }
